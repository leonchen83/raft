...
import static com.moilioncircle.raft.TestUtil.newTestConfig;
...
public class RawNodeTest {
    ...
    @Test
    public void testRawNodeStep() {
        for (MessageType msgt : MessageType.values()) {
            Storage.MemoryStorage s = new Storage.MemoryStorage();
            RawNode.Peer peer = new RawNode.Peer();
            peer.id = 1;
            RawNode rawNode = new RawNode(newTestConfig(1, Lists.of(), 10, 1, s), Lists.of(peer));
            ...
            try {
                ...
                if (isLocalMsg(msgt)) {
                    if (e != ERR_STEP_LOCAL_MSG) {
                        fail(msgt + ": step should ignore ");
                    }
                }
            }
        }
